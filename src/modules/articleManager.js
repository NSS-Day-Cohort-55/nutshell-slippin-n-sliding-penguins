...
export const getAllArticles = () => {
  return fetch(`${remoteURL}/articles?_sort=timestamp&_order=desc`).then(
    (res) => res.json()
  );
};
...
export const getArticlesByUser = (num) => {
  return fetch(`${remoteURL}/articles?userId=${num}`).then((res) => res.json());
};

export const getSpecificArticle = (num) => {
  return fetch(`${remoteURL}/articles/${num}`).then((res) => res.json());
};
